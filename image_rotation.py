...
while(i<360):
    ...
    cv2.imwrite("Rotations/User."+image[:length-4]+"."+str((i/5)+1)+".jpg",img_rotation)
...
rotation_matrix = cv2.getRotationMatrix2D((num_cols/2, num_rows/2), median*5, 1)
img_rotation = cv2.warpAffine(img, rotation_matrix, (num_cols, num_rows))
gray = cv2.cvtColor(img_rotation, cv2.COLOR_BGR2GRAY)
faces = face_cascade.detectMultiScale(gray, 1.2, 5)
for (x,y,w,h) in faces:
    cv2.rectangle(img_rotation,(x,y),(x+w,y+h),(255,0,0),2)
for (x,y,w,h) in faces:
    cv2.imwrite("outputImage/"+str(median)+'_'+image,img_rotation[y:y+h,x:x+w])
cv2.imshow('Rotation', img_rotation)
cv2.waitKey(0)
